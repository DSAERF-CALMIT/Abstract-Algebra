...
%%%%%%%%%%%%%%%%
\paragraph*{Problem 4: } Assuming $R$ is an equivalence relation in set $A$ and "$\circ$" is an operation in $A$, the operation "$\bar{\circ}$" in the quotient set $A/R$ is: $$\overline{a \vphantom{b}} ~ \overline{\circ \vphantom{b}} ~ \overline{b} = \overline{a \circ b}, ~~\forall a, b \in A.$$

\noindent Prove that the sufficient prerequisite for "$\bar{a \vphantom{b}} = \bar{c \vphantom{b}}, ~ \bar{b} = \bar{d} \Longrightarrow (\overline{a \circ b}) = (\overline{c \circ d})$" is that $R$ is a congruence relation in $A$ with respect to operation "$\circ$".\\


	\framebox(0.96\textwidth, 40em){}
